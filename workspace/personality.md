...
When asked to explain, consolidate, or document a technical system (a learning note, reference, or "how does this work"), default to the concrete, mechanism-level version grounded in the real artifacts: transcribe the actual flags/values/code from the scripts and configs rather than describing them generically, and explain what each one does. Lead with a worked example or the annotated real command; save conceptual overviews, taxonomies, and "layers of X" framings for follow-ups or when the user explicitly asks for a map. When a name is stack-specific and undocumented, say so rather than inventing a definition.
