...
export function buildAllVaultNotes(
  ...
  const resolvedAnswers = new Map<string, { resolvedText: string; citedSourceIndices: Set<number> }>();
  const allCitedSourceIndices = new Set<number>();

  for (const [question, response] of Object.entries(ctx.answers)) {
    const preResolved = resolvedMap.get(question);

    let result: { resolvedText: string; citedSourceIndices: Set<number> };
    if (preResolved) {
      result = {
      ...
      result = resolveAnswerCitations(response.text, ctx.sources);
    }

    resolvedAnswers.set(question, result);
    for (const idx of result.citedSourceIndices) {
      allCitedSourceIndices.add(idx);
    }
  }

  // ── 2. Source notes ──────────────────────────────────────────────────────────
  // Each cited source gets a single passage anchor: ## Passage N where N = sourceIndex + 1.
  // This matches the [[Source Title#Passage N]] wikilinks generated by resolveAnswerCitations.
  ...
  for (const [question] of Object.entries(ctx.answers)) {
    ...
    const citedSources = Array.from(resolved.citedSourceIndices)
      .sort((a, b) => a - b)
